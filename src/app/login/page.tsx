...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex bg-surface-50 w-full relative">
      {/* Left decorative panel */}
      <div className="hidden lg:flex lg:w-1/2 bg-brand-50 items-center justify-center p-12 relative border-r border-surface-200">
        <div className="relative z-10 max-w-lg">
          <div className="w-16 h-16 rounded-2xl bg-brand-600 shadow-md flex items-center justify-center mb-8">
          ...
          <h2 className="text-4xl font-bold text-surface-900 mb-4 leading-tight">
            Collab Force Health System
          </h2>
          <p className="text-lg text-surface-600 leading-relaxed">
            Streamlined patient and appointment management 
            for modern clinical teams. Secure, fast, and highly structured.
          </p>
          <div className="mt-12 grid grid-cols-3 gap-6 pt-8 border-t border-brand-200">
            {[
              { value: "256-bit", label: "Encryption" },
              { value: "Compliance", label: "Architecture" },
              { value: "Solid", label: "UX Framework" },
            ].map((stat) => (
              <div key={stat.label}>
                <p className="text-xl font-extrabold text-brand-700">{stat.value}</p>
                <p className="text-xs font-semibold uppercase tracking-wider text-surface-500 mt-1">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
